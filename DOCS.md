...
5. **Data.gov** — Only in Tauri. Data & sources view calls `fetchDataGovRecentCsv` / `fetchUkDataRecentCsv` with optional `query`, `sort`, and `rows` (CKAN `package_search`, up to 200 rows). Sidebar adds catalog search, sort, max results, and a client-side page filter. “Save to folder” → `saveCsvToFolder`.
